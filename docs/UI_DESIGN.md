...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.1 | 2026-05-31 | MVP UI：登录/注册/仪表盘/账号/帖子/日历/列表 |
| v0.2 | 2026-06-01 | 外部 API Key 管理 UI（设置页） |
| v0.3 | 2026-06-01 | 回收站页面（软删除 / 恢复 / 永久删除二次确认） |
| v0.4 | 2026-06-02 | AI tools 页（MCP 配置 + 工具列表 + Scope 矩阵） |
| v0.4.3 | 2026-06-03 | 缩略图 force regenerate 入口 + 编辑模式原图展示 |
| v0.4.7 | 2026-06-03 | 帖子列表 / 回收站 / 统计页健壮性提升 |
| **v0.4.8** | **2026-06-04** | **端口单一源**：AI tools 页的 MCP 配置示例和端点展示从 `src/lib/config.ts` 的 `getMcpEndpointUrl()` 派生，dev/prod 切域名只改 `APP_URL`；默认端口 3000 → 3456 |
...
*文档生成时间：2026-05-31*
*最后更新：2026-06-04（v0.4.8 端口单一源：AI tools 页 MCP 端点从 helper 派生）*