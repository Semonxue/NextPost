...
| **模块** | 内容创作 |
| **前置条件** | 用户已登录，存在帖子 ID `post-001` |
| **测试步骤** | 1. 访问帖子编辑页<br>2. 点击「删除」按钮<br>3. 在确认弹窗中点击「确认」 |
| **预期结果** | 1. 删除成功，跳转到列表页<br>2. 帖子不再出现在列表中<br>3. 数据库 `Post` 表该记录已删除 |
...
| **模块** | 媒体上传 |
| **前置条件** | 用户已登录 |
| **测试步骤** | 1. 进入帖子创建/编辑页<br>2. 点击媒体上传区域<br>3. 选择一个 JPG 图片文件 (< 10MB)<br>4. 上传 |
| **预期结果** | 1. 图片预览显示<br>2. 媒体 URL 已添加到帖子<br>3. 数据库 `Media` 表新增记录 |
...
| **模块** | 媒体上传 |
| **前置条件** | 用户已登录 |
| **测试步骤** | 1. 进入帖子创建/编辑页<br>2. 点击媒体上传区域<br>3. 选择一个 MP4视频文件 (< 50MB)<br>4. 上传 |
| **预期结果** | 1. 视频预览/缩略图显示<br>2. 媒体 URL 已添加到帖子<br>3. 数据库 `Media` 表新增记录 |

#### TC-MEDIA-003：上传文件 - 文件过大
...
| **模块** | 媒体上传 |
| **前置条件** | 用户已登录 |
| **测试步骤** | 1. 进入帖子创建/编辑页<br>2. 点击媒体上传区域<br>3. 选择一个 20MB 的图片文件<br>4. 上传 |
| **预期结果** | 1. 显示错误提示「文件大小不能超过 10MB」<br>2. 上传失败 |

#### TC-MEDIA-004：上传文件 - 不支持的格式
...
| **模块** | 媒体上传 |
| **前置条件** | 用户已登录 |
| **测试步骤** | 1. 进入帖子创建/编辑页<br>2. 点击媒体上传区域<br>3. 选择一个 .exe 文件<br>4. 上传 |
| **预期结果** | 1. 显示错误提示「不支持的文件格式」<br>2. 上传失败 |
...
| **标题** | 删除媒体 |
| **模块** | 媒体上传 |
| **前置条件** | 用户已登录，帖子关联了媒体文件 |
| **测试步骤** | 1. 进入帖子编辑页<br>2. 找到已上传的媒体<br>3. 点击删除按钮 |
| **预期结果** | 1. 媒体从帖子中移除<br>2. 数据库 `Media` 表记录保留（或标记删除） |
