...
| **v0.4.3** | **2026-06-03** | **缩略图 force regenerate**：维护接口 `POST /api/maintenance/regenerate-thumbnails` 支持 `force=true` 重新生成所有缩略图 |
| **v0.4.4** | **2026-06-03** | **MediaUploader 增强**：编辑模式下展示原图，缩略图处理优化 |
...
| **v0.6.0** | **2026-06-18** | **Prisma → Drizzle ORM 迁移 + CF Workers 部署**：数据库层从 Prisma v5 全面迁移到 Drizzle ORM（`drizzle-orm` + `@libsql/client`），10 张表 schema 重建。本地开发 SQLite（`@libsql/client`），CF Workers 用 D1 adapter。移除 `pnpm db:generate`。Cloudflare Pages 废弃，改 Workers + OpenNext。新增 `src/lib/db/`（`schema.ts` + `index.ts` 双模式 db client）。API/认证/页面全适配 Drizzle。详见 [CLOUDFLARE_DEPLOY.md](./CLOUDFLARE_DEPLOY.md)。 |
...
| 数据库 | Drizzle ORM + SQLite | 环境变量切换（本地 SQLite/CF D1） |
...
每个平台有两个标识：
- **key**：小写唯一标识符，用于数据库存储（如 `twitter`）
- **label**：显示名称，供 UI 使用（如 `Twitter / X`）

### 当前已支持平台

| key | label | 说明 |
|-----|-------|------|
| twitter | Twitter / X | 最早的社交平台，支持短文本（280字符） |
| xiaohongshu | 小红书 | 支持长文本（1000字符）和图片（18张） |
| instagram | Instagram | 图片分享平台，支持图文混合（10张图片） |

### 新增平台操作步骤

当需要添加新的社交平台时，需要修改以下文件：

#### 1. 修改 `src/lib/platform.ts`

在 `REGISTERED_PLATFORMS` 数组中添加新平台（key 小写，label 中文或英文）：
...
export const REGISTERED_PLATFORMS: PlatformDefinition[] = [
  ...
  // 新增平台，例如：
  { key: "facebook", label: "Facebook", icon: "/icons/facebook.svg" },
];
```

在 `DEFAULT_PLATFORM_CONFIG` 中添加新平台的配置（key 小写）：
...
#### 2. 修改 `src/lib/platform-style.ts`

在 `STYLES` 对象中添加新平台的样式配置（key 小写）：
...
#### 3. 修改 `src/lib/db/seed.ts`

在 `configs` 数组中添加新平台的配置（name 使用 key 小写）：
...
如果新平台有图标需求：
- 创建 SVG 图标文件，放在 `public/icons/` 目录下
- 图标路径格式：`/icons/<platform-key>.svg`
...
```sql
-- 插入平台记录（name 使用 key 小写）
INSERT INTO "Platform" (id, name, icon, "createdAt") 
VALUES ('<unique-id>', 'facebook', '/icons/facebook.svg', datetime('now'));

-- 插入平台配置
INSERT INTO "PlatformConfig" (id, "platformId", "maxContentLength", "maxImages", "maxVideos", "allowMixedMedia", "createdAt", "updatedAt")
SELECT '<config-id>', p.id, 63206, 10, 1, 1, datetime('now'), datetime('now')
FROM "Platform" p WHERE p.name = 'facebook';
```

### 删除平台

如果需要删除某个平台，需要同时清理以下数据：

```sql
-- 删除平台配置
DELETE FROM "PlatformConfig" WHERE "platformId" IN (SELECT id FROM "Platform" WHERE name = 'linkedin');

-- 删除该平台下的账号
DELETE FROM "Account" WHERE "platformId" IN (SELECT id FROM "Platform" WHERE name = 'linkedin');

-- 删除平台记录
DELETE FROM "Platform" WHERE name = 'linkedin';
```

同时需要从代码中移除对应的配置（platform.ts、platform-style.ts 和 seed.ts）。
...
| **Drizzle ORM** | 轻量级 ORM，支持 SQLite/D1/PostgreSQL，类型安全 |
...
*文档生成时间：2026-05-31*
*最后更新：2026-06-18（v0.6.0 Prisma → Drizzle ORM 迁移 + CF Workers 部署）*