...
| **v0.4** | **2026-06-02** | **外部 MCP 写能力 MVP**：新增 `upload_media_from_url` / `create_post` / `update_post` 三个写工具；引入 Scope 权限系统（`read` / `write` / `read_write` 三档）；写工具受字段白名单 + 状态锁双重保护 |
...
| 10b | **外部 MCP 写能力 MVP（v0.4）** | 新增 `upload_media_from_url` / `create_post` / `update_post` 三个写工具，让外部 AI 直接在 NextPost 排期 | P1 |
